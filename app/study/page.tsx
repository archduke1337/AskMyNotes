...
export default function StudyPage() {
  ...
  }, [items, activeSubjectId, revealed]);
    ...
    <div className="flex flex-col flex-1 bg-bg-app font-sans relative">
      ...
      <div className="h-12 border-b border-border-strong flex items-center justify-between px-4 md:px-6 shrink-0 bg-bg-surface relative z-10">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-mono tracking-widest uppercase font-bold">STUDY REGIMEN</h2>
        </div>
        <div className="flex items-center gap-2 md:gap-4">
          {/* Subject selector (compact) */}
          <div className="relative border border-border-strong bg-bg-surface">
            <select
              className="appearance-none bg-transparent py-1 pl-3 pr-7 text-base font-mono font-bold uppercase tracking-widest focus:outline-none cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-2 border border-border-strong hover:bg-text-primary hover:text-bg-app px-3 py-1 font-mono text-base uppercase tracking-widest cursor-pointer disabled:opacity-50"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10 max-w-5xl mx-auto w-full">
        {error && (
          <div className="border border-danger/30 bg-danger/5 p-3 text-base font-mono text-danger mb-6">
            {error}
        ...
        {loadingItems ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-base font-mono tracking-widest uppercase text-text-tertiary mb-2">NO STUDY ITEMS YET</p>
            <p className="text-base font-mono text-text-tertiary mb-4">Upload notes and click GENERATE to create study questions.</p>
            <button onClick={handleGenerate} disabled={generating || !activeSubjectId} className="px-4 py-2 bg-text-primary text-bg-app text-base font-mono tracking-widest uppercase font-bold disabled:opacity-50 cursor-pointer flex items-center gap-2">
          ...
          <>
            ...
            {mcqItems.length > 0 && (
              <div className="mb-12">
                <div className="border-b border-border-default pb-2 mb-6 flex items-center justify-between">
                  <h3 className="font-mono text-base font-bold uppercase tracking-widest">MULTIPLE CHOICE</h3>
                  <span className="text-base font-mono tracking-widest text-text-tertiary">{mcqItems.length} ITEM(S)</span>
                </div>
                <div className="space-y-6">
                  {mcqItems.map((item, idx) => {
                    ...
                    return (
                      <div key={item.$id} className="gsap-hover border border-border-strong bg-bg-surface p-4 md:p-6">
                        <div className="flex justify-between items-start mb-4 border-b border-border-strong pb-3">
                          <span className="bg-text-primary text-bg-app font-mono text-base font-bold px-2 py-1 tracking-widest uppercase">Q.{idx + 1}</span>
                          <button onClick={() => handleDelete(item.$id)} className="text-[10px] font-mono text-text-tertiary hover:text-danger cursor-pointer">DELETE</button>
                        </div>
                        <p className="text-base font-mono uppercase tracking-wider leading-relaxed mb-6">{content.question}</p>
                        <div className="space-y-3 font-mono text-base uppercase">
                          {(content.options || []).map((opt, oi) => {
                            ...
                            return (
                              <button
                                ...
                                className={`flex items-center gap-4 border p-3 md:p-4 w-full text-left cursor-pointer transition-colors ${isRevealed && isCorrect ? "border-text-primary bg-bg-subtle border-2" :
                                  isRevealed && isSelected && !isCorrect ? "border-danger bg-danger/5" :
                                    "border-border-strong hover:bg-bg-subtle"
                                  }`}
                              >
                                <div className={`w-6 h-6 border flex items-center justify-center shrink-0 tracking-widest ${isRevealed && isCorrect ? "bg-text-primary text-bg-app border-text-primary" : "border-border-strong"
                                  }`}>{letter}</div>
                        ...
                        {isRevealed && content.explanation && (
                          <div className="mt-6 border-t border-border-strong border-dashed pt-4">
                            <h4 className="font-mono text-base font-bold tracking-widest uppercase mb-2">EXPLANATION</h4>
                            <p className="text-base font-mono uppercase tracking-wider leading-relaxed text-text-secondary">{content.explanation}</p>
                          </div>
                        )}
                      </div>
                    );
            ...
            {shortItems.length > 0 && (
              <div className="mb-12">
                <div className="border-b border-border-default pb-2 mb-6 flex items-center justify-between">
                  <h3 className="font-mono text-base font-bold uppercase tracking-widest">SHORT ANSWER</h3>
                  <span className="text-base font-mono tracking-widest text-text-tertiary">{shortItems.length} ITEM(S)</span>
                </div>
                <div className="space-y-6">
                  {shortItems.map((item, idx) => {
                    ...
                    return (
                      <div key={item.$id} className="gsap-hover border border-border-strong bg-bg-surface p-4 md:p-6">
                        <div className="flex justify-between items-start mb-4 border-b border-border-strong pb-3">
                          <span className="font-mono text-base font-bold tracking-widest uppercase">Q.{idx + 1}</span>
                          <div className="flex items-center gap-3">
                            <button onClick={() => toggleReveal(item.$id)} className="flex items-center gap-2 font-mono text-base tracking-widest font-bold border border-border-strong px-2 py-1 uppercase hover:bg-bg-subtle cursor-pointer">
                        ...
                        <div className="border border-border-strong relative">
                          <textarea
                            className="w-full bg-transparent p-4 min-h-[100px] font-mono text-base uppercase tracking-wider focus:outline-none focus:bg-bg-subtle placeholder:text-text-tertiary resize-none"
                        ...
                        {isRevealed && (
                          <div className="mt-6 border border-border-strong border-dashed bg-bg-subtle p-4 md:p-6">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <h4 className="font-mono text-base font-bold tracking-widest uppercase">MODEL ANSWER</h4>
                            </div>
                            <p className="text-base font-mono uppercase tracking-wider leading-relaxed text-text-secondary">{content.answer}</p>
                            {content.explanation && (
                              <p className="text-base font-mono uppercase tracking-wider leading-relaxed text-text-tertiary mt-3 pt-3 border-t border-border-default">{content.explanation}</p>
                            )}
                          </div>
                        )}
                      </div>
