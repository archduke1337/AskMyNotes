    ...
    <div className="flex flex-col h-full bg-bg-app font-sans relative">
      ...
      <div className="h-12 border-b border-border-strong flex items-center justify-between px-4 md:px-6 shrink-0 bg-bg-surface relative z-10">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-semibold tracking-wide">Study Mode</h2>
        </div>
        <div className="flex items-center gap-2 md:gap-4">
          {/* Subject selector (compact) */}
          <div className="relative border border-border-strong bg-bg-surface">
            <select
              className="appearance-none bg-transparent py-1 pl-3 pr-7 text-sm font-semibold tracking-wide focus:outline-none cursor-pointer"
          ...
          <button
            ...
            className="flex items-center gap-2 border border-border-strong hover:bg-text-primary hover:text-bg-app px-3 py-1 font-semibold text-sm tracking-wide cursor-pointer disabled:opacity-50 transition-colors"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10 max-w-5xl mx-auto w-full">
        {error && (
          <div className="border border-danger/30 bg-danger/5 p-3 text-sm font-medium text-danger mb-6">
            {error}
        ...
        {loadingItems ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-sm font-semibold tracking-wide text-text-tertiary mb-2">No study questions yet</p>
            <p className="text-xs text-text-tertiary mb-4">Upload notes and click Generate to create study questions.</p>
            <button onClick={handleGenerate} disabled={generating || !activeSubjectId} className="px-4 py-2 bg-text-primary text-bg-app text-sm font-semibold tracking-wide rounded disabled:opacity-50 cursor-pointer flex items-center gap-2">
              {generating ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} Generate Questions
            </button>
          </div>
        ) : (
          <>
            ...
            {mcqItems.length > 0 && (
              <div className="mb-12">
                <div className="border-b border-border-default pb-2 mb-6 flex items-center justify-between">
                  <h3 className="font-semibold text-sm tracking-wide">Multiple Choice</h3>
                  <span className="text-xs font-medium text-text-tertiary">{mcqItems.length} questions</span>
                </div>
                    ...
                    return (
                      <div key={item.$id} className="border border-border-strong bg-bg-surface p-4 md:p-6">
                        <div className="flex justify-between items-start mb-4 border-b border-border-strong pb-3">
                          <span className="bg-text-primary text-bg-app text-xs font-semibold px-2 py-1 tracking-wide rounded">Q.{idx + 1}</span>
                          <button onClick={() => handleDelete(item.$id)} className="text-xs font-medium text-text-tertiary hover:text-danger cursor-pointer">Delete</button>
                        </div>
                        <p className="text-sm font-medium tracking-wide leading-relaxed mb-6">{content.question}</p>
                        ...
                        {isRevealed && content.explanation && (
                          <div className="mt-6 border-t border-border-strong border-dashed pt-4">
                            <h4 className="text-xs font-semibold tracking-wide mb-2">Explanation</h4>
                            <p className="text-sm font-medium tracking-wide leading-relaxed text-text-secondary">{content.explanation}</p>
                          </div>
                        )}
                      </div>
                    );
            ...
            {shortItems.length > 0 && (
              <div className="mb-12">
                <div className="border-b border-border-default pb-2 mb-6 flex items-center justify-between">
                  <h3 className="font-semibold text-sm tracking-wide">Short Answer</h3>
                  <span className="text-xs font-medium text-text-tertiary">{shortItems.length} questions</span>
                </div>
                <div className="space-y-6">
                  {shortItems.map((item, idx) => {
                    ...
                    return (
                      <div key={item.$id} className="border border-border-strong bg-bg-surface p-4 md:p-6">
                        <div className="flex justify-between items-start mb-4 border-b border-border-strong pb-3">
                          <span className="text-xs font-semibold tracking-wide">Q.{idx + 1}</span>
                          <div className="flex items-center gap-3">
                            <button onClick={() => toggleReveal(item.$id)} className="flex items-center gap-2 text-xs font-semibold tracking-wide border border-border-strong px-2 py-1 rounded hover:bg-bg-subtle cursor-pointer">
                              <Eye className="w-4 h-4" /> {isRevealed ? "Hide" : "Reveal"}
                            </button>
                            <button onClick={() => handleDelete(item.$id)} className="text-xs font-medium text-text-tertiary hover:text-danger cursor-pointer">Delete</button>
                          </div>
                        </div>
                        <p className="text-sm font-medium tracking-wide leading-relaxed mb-6">{content.question}</p>
                        <div className="border border-border-strong relative rounded">
                          <textarea
                            className="w-full bg-transparent p-4 min-h-25 text-sm focus:outline-none focus:bg-bg-subtle placeholder:text-text-tertiary resize-none rounded"
                        ...
                        {isRevealed && (
                          <div className="mt-6 border border-border-strong border-dashed bg-bg-subtle p-4 md:p-6">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <h4 className="text-xs font-semibold tracking-wide">Model Answer</h4>
                            </div>
                            <p className="text-sm font-medium tracking-wide leading-relaxed text-text-secondary">{content.answer}</p>
                            {content.explanation && (
                              <p className="text-sm tracking-wide leading-relaxed text-text-tertiary mt-3 pt-3 border-t border-border-default">{content.explanation}</p>
                            )}
                          </div>
                        )}
                      </div>
